import threading
from data.parametrs.get_data import get_result, id
import discord
import datetime
import asyncio
import random
...
from data.parametrs.get_data import get_result, list_words
...
for i in range(len(list_words)):
    letter = str(list_words[0][i]) + " " + list_words[1][i]
    ...
    if str(message.channel) in channels_for_commands and str(message.author) in true_users:
        if message.content.startswith("!hello"):
            ...
            if message.content.find("!list 1") != -1:
                await channel.send('http://127.0.0.1:8080/year/1')
                app.run(port=8080, host='127.0.0.1')
        elif message.content.startswith("!get_url"):
            parametr = message.content[9:].split(" ")
            if parametr[0] == '1':
                await channel.send(
                    list_words[1][int(parametr[1]) - 1] + " " + "-" + " " + list_words[2][int(parametr[1]) - 1])
